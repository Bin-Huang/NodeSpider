"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
const ava_1 = require("ava");
const delay = require("delay");
const fs = require("fs-extra");
const path = require("path");
const jsonPipe_1 = require("../../src/pipe/jsonPipe");
ava_1.default("test 1 for jsonPipe", async (t) => {
    const filepath = path.resolve(__dirname, "jsonPipe_test1.json");
    const pipe = jsonPipe_1.default({
        name: "test1",
        path: filepath,
        items: ["item1", "item2"],
    });
    pipe.write(["data1", "data2"]);
    pipe.write(["data3", null]);
    pipe.write(["data4", "data5"]);
    pipe.end();
    await delay(1000);
    t.is(pipe.name, "test1");
    const str = await fs.readFile(filepath, { encoding: "utf8" });
    t.is(str, `[
{
    "item1": "data1",
    "item2": "data2"
},
{
    "item1": "data3",
    "item2": null
},
{
    "item1": "data4",
    "item2": "data5"
}
]`);
    await fs.remove(filepath);
});
ava_1.default("test 2 for jsonPipe", async (t) => {
    const filepath = path.resolve(__dirname, "jsonPipe_test2.json");
    const pipe = jsonPipe_1.default({
        name: "test1",
        path: filepath,
        items: {
            item1: (a) => a,
            item2: (a) => a,
        },
    });
    pipe.write([null, null]);
    pipe.write(["data3", null]);
    pipe.write(["data4", "data5"]);
    pipe.end();
    await delay(1000);
    t.is(pipe.name, "test1");
    const str = await fs.readFile(filepath, { encoding: "utf8" });
    t.is(str, `[
{
    "item1": null,
    "item2": null
},
{
    "item1": "data3",
    "item2": null
},
{
    "item1": "data4",
    "item2": "data5"
}
]`);
    await fs.remove(filepath);
});
//# sourceMappingURL=jsonPipe.test.js.map