...
- [若有异步, 必须使用 async]
- [若为异步，resolve传递currentTask] resolve 必须传递1个参数作为结果: currentTask 当前任务信息, 其中文档操作符可以是自带的jq，也可以是 plantom的 page， 反正两者不兼容(当使用plantom时，再使用自带jq选择器是没有意义的)
- [若为异步, reject传递error]如果预处理失败，reject 必须返回 error，以此直接报错停止
